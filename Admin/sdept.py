...
db = MySQLdb.connect("localhost","root","liitliit","school" )
cursor = db.cursor()
sql="select StudId,Sname,Addr,DOB,DOJ,CName,FatherName,MotherName,Remarks from stud s,courses c where s.CourseId=c.CourseId AND c.DeptId='%s'"%(DeptId)
cursor.execute(sql)
...
print "<tr><td>SName : </td><td><input type=text name='StudId'></td></tr>"
...
cursor = db.cursor()
sql="select CourseId,Cname from courses where DeptId='%s'"%(DeptId)
cursor.execute(sql)
list2d = [list(i) for i in cursor.fetchall()]
