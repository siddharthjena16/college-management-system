...
db = MySQLdb.connect("localhost","root","liitliit","school" )
cursor = db.cursor()
sql="select StudId,Sname,Addr,DOB,DOJ,CName,FatherName,MotherName,Remarks from stud s,courses c where s.CourseId=c.CourseId AND s.StudId='%s'"%(StudId)
cursor.execute(sql)
...
for row in list2d:
    ...
    newrow = u'<tr>'
    if i!=0:
        newrow += u'<td align="center" style="padding:1px 4px"><input type=radio name="StudI" value="'+unicode(row[0])+'"'+u'</td>'
    elif i==0:
...
cursor=db.cursor()
sql="select Sname from stud where StudId='%s'"%(StudId) 
cursor.execute(sql)
list2d = [list(i) for i in cursor.fetchall()]
sname="lol";
for row in list2d:
    sname=unicode(row[0])
print "<tr><td>SName : </td><td>"+sname+"<input type=text name='StudId' value='%s' style='Display:none'>"%(StudId)+"</td></tr>"
